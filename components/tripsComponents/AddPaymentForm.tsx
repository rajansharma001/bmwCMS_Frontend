...
import Button from "../Button"; // Assuming the Button component is in this path
import { formInput, inputLable, inputHelp } from "@/styles/styles";

// Define the expected props for the component
interface AddPaymentFormProps {
  tripId: string;
  currentBalance: number | undefined; // The balance due on the trip
...
export const AddPaymentForm = ({
  ...
  const handlePaymentSubmit = async (e: FormEvent) => {
    ...
    // Check for negative balance (Trip is already overpaid or fully paid)
    if (balance <= 0 && formData.amount > 0) {
      toast.warning(
        `This trip is already fully paid or overpaid (Balance: Rs. ${balance.toFixed(
          2
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/trips/add-payment/${tripId}`,
        {
          method: "POST",
          credentials: "include",
          headers: {
